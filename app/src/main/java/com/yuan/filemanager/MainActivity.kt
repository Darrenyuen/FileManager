...
class MainActivity : AppCompatActivity(), FileDialogFragment.Callback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pathTextView.text = "/"
        list = FileUtil.getGroupFiles("/").sorted()
        ...
        listView.setOnItemClickListener { adapterView, view, i, l ->
            ...
            if (File(item.toString()).isDirectory)  {
                ...
                list = FileUtil.getGroupFiles(item.toString()).sorted()
                listView.adapter = FileItemAdapter(
                    this,
                    R.layout.item_file,
                    list!!
...
//                TODO("拿到nowfilename") //To change body of created functions use File | Settings | File Templates.
    ...
    private fun searchFile(fileName: String, list: List<String>?) {
        if (fileName.equals("")) {
            Toast.makeText(this, "文件名不能为空", Toast.LENGTH_SHORT).show()
        }
        list?.forEach { it -> it.substringAfterLast("/") }
        list?.filter { it.contains(fileName) }?.forEach { it ->
            Log.d(TAG, it)
        }
    }
